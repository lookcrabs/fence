...
def _expand_base_url(app):
    ...
    if server_name:
        ...
        app.config["SERVER_NAME"] = provided_value.replace(
            "{{BASE_URL}}", app.config["BASE_URL"]
        )

    google_redirect = (
        app.config.get("OPENID_CONNECT", {}).get("google", {}).get("redirect_url")
    )
    if google_redirect:
        provided_value = app.config["OPENID_CONNECT"]["google"]["redirect_url"]
        app.config["OPENID_CONNECT"]["google"]["redirect_url"] = provided_value.replace(
            "{{BASE_URL}}", app.config["BASE_URL"]
        )
    ...
    if default_logout:
        ...
        app.config["DEFAULT_LOGIN_URL"] = provided_value.replace(
            "{{BASE_URL}}", app.config["BASE_URL"]
        )
    ...
    if shib_url:
        ...
        app.config["SSO_URL"] = provided_value.replace(
            "{{BASE_URL}}", app.config["BASE_URL"]
        )
    ...
    if access_token_url:
        provided_value = app.config["OPENID_CONNECT"]["fence"]["client_kwargs"][
            "redirect_uri"
        ]
        app.config["OPENID_CONNECT"]["fence"]["client_kwargs"][
            "redirect_uri"
...
def _expand_api_base_url(app):
    ...
    """
    api_base_url = (
        app.config.get("OPENID_CONNECT", {}).get("fence", {}).get("api_base_url")
    )
    if api_base_url is not None:
        authorize_url = (
            app.config.get("OPENID_CONNECT", {}).get("fence", {}).get("authorize_url")
        )
        if authorize_url:
            provided_value = app.config["OPENID_CONNECT"]["fence"]["authorize_url"]
            app.config["OPENID_CONNECT"]["fence"][
                "authorize_url"
        ...
        if access_token_url:
            provided_value = app.config["OPENID_CONNECT"]["fence"]["access_token_url"]
            app.config["OPENID_CONNECT"]["fence"][
                "access_token_url"
            ] = provided_value.replace("{{api_base_url}}", api_base_url)
        ...
        if refresh_token_url:
            provided_value = app.config["OPENID_CONNECT"]["fence"]["refresh_token_url"]
            app.config["OPENID_CONNECT"]["fence"][
                "refresh_token_url"
            ] = provided_value.replace("{{api_base_url}}", api_base_url)
